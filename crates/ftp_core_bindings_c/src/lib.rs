...
pub unsafe extern "C" fn ftp_compute(handle: *mut FtpHandle, method: i32) -> i32 {
    ...
    let compute_method = match method {
        ...
        _ => {
            set_last_error(format!("unknown method: {} (expected 0=Stock, 1=Flux)", method));
            return -1;
        }
    };
...
#[no_mangle]
pub unsafe extern "C" fn ftp_get_stock_amort(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
    copy_matrix((*handle).inner.stock_amort(), "stock_amort", out_buf, buf_len)
}

/// Copies stock_instal into `out_buf`. Returns 0 on success, -1 on error.
#[no_mangle]
pub unsafe extern "C" fn ftp_get_stock_instal(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
    copy_matrix((*handle).inner.stock_instal(), "stock_instal", out_buf, buf_len)
}

/// Copies varstock_amort into `out_buf`. Returns 0 on success, -1 on error.
#[no_mangle]
pub unsafe extern "C" fn ftp_get_varstock_amort(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
    copy_matrix((*handle).inner.varstock_amort(), "varstock_amort", out_buf, buf_len)
}

/// Copies varstock_instal into `out_buf`. Returns 0 on success, -1 on error.
#[no_mangle]
pub unsafe extern "C" fn ftp_get_varstock_instal(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
    copy_matrix((*handle).inner.varstock_instal(), "varstock_instal", out_buf, buf_len)
}

/// Copies ftp_rate into `out_buf`. Returns 0 on success, -1 on error.
#[no_mangle]
pub unsafe extern "C" fn ftp_get_ftp_rate(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
...
#[no_mangle]
pub unsafe extern "C" fn ftp_get_ftp_int(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
...
#[no_mangle]
pub unsafe extern "C" fn ftp_get_market_rate(handle: *const FtpHandle, out_buf: *mut f64, buf_len: i32) -> i32 {
    if handle.is_null() { set_last_error("null handle".into()); return -1; }
    copy_matrix((*handle).inner.market_rate(), "market_rate", out_buf, buf_len)
}
...
mod tests {
    ...
    fn test_null_handle_returns_error() {
        unsafe {
            ...
            assert_eq!(ftp_get_stock_amort(std::ptr::null(), buf.as_mut_ptr(), 10), -1);
